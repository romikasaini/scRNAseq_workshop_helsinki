...
# WARNING: Might find typos and stupic mistakes :D
...
#We are preprocessing the datasets with shortcut now. But please do your preprocessing carefully. As you know this dataset has perfect quality, we are saving time here.

for (file in c("data1", "data2", "data3", "data4")) {
  seurat_data <-Read10X(data.dir = file)
  seurat_obj <-CreateSeuratObject(counts = seurat_data, min.features = 200, min.cells = 3, project = file, assay = "RNA")
  seurat_obj[["percent.mt"]] <- PercentageFeatureSet(seurat_obj, pattern = "^MT-")
  assign(file, seurat_obj)
}

#let's make a list of these datasets
...
filtered_data <- all_data #we need this list later for merge function
...
integrate_data <- RunPCA(integrate_data, npcs = 20, verbose = FALSE)
integrate_data <- RunUMAP(integrate_data, reduction = "pca", dims = 1:20)
integrate_data <- FindNeighbors(integrate_data, reduction = "pca", dims = 1:20)
...
merged_data <- ScaleData(merged_data, features = var.genes)
...
merged_data <- RunUMAP(merged_data, dims = 1:20, reduction = "pca", n.neighbors = 15, min.dist = 0.5, spread = 1, metric = "euclidean", seed.use = 1)  
...
#now let's try another methods called harmony
#author: Terézia Kurucová
merged_harmony <- merged_data %>% RunHarmony("orig.ident", plot.convergence=T)
harmony_embeddings <- Embeddings(merged_harmony, "harmony")

merged_harmony <- merged_harmony %>% RunUMAP(dims = 1:20, label=T, reduction="harmony") %>% FindNeighbors(dims = 1:20, k.param = 20, reduction="harmony") %>% FindClusters(resolution = 0.5) %>% identity()

p7 <- DimPlot(merged_harmony, reduction = "umap", group.by = "orig.ident") + ggtitle("harmony")
p7
p8 <- DimPlot(merged_harmony, reduction = "umap", split.by = "orig.ident") + ggtitle("harmony")
p1 + p6 + p7

#let's discuss and write comments together:
#Which package do you think did a better job, why?

#Note: 
#data1 and data2 are samples from same patient
#data1 and data2 are from the same batch, data3 and data4 are from the same batch
#note also that data3 has more number of cells to begin with
